...
public class DHeap<K extends Comparable<K>,D> implements PriorityQueue<K,D>
{
	...
	*/
	public void delete(PriorityQueueNode<K,D> node)
	{
		if(n>0)
		{
			DHeapNode<K,D> u = (DHeapNode<K,D>) nodes.get(n-1);
			nodes.remove(n-1);
			n--;
			
			if(u!=(DHeapNode<K,D>)node)
			{				
				int v =((DHeapNode<K,D>) node).index; //indice nodo da eliminare
				nodes.set(v,u); //inserisce u al posto del nodo da eliminare
				u.index = v; //sistema il campo index del nodo spostato
	...
	*/	
	...
	{
		DHeapNode<K,D> v=(DHeapNode<K,D>) node;
		
		if(newKey.compareTo(v.key)>0)
		{
			v.key=newKey;
			moveDown(v);
		}
